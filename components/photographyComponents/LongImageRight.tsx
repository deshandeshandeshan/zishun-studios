...
type longImageRightProps = Extract<
  NonNullable<NonNullable<SINGLE_SELECTED_WORK_QUERYResult>["content"]>[number],
  { _type: "longImageRight" }
>;

export function LongImageRight({ leftImage, rightImage }: longImageRightProps) {
  ...
  return (
    <section className="long-image-right grid mobile-padding">
      <div className="long-image-right-left-image">
        {leftImage ? (
          <Image
            onClick={() =>
              setActiveImage({
            ...
            height={1920}
            alt={leftImage.alt || ""}
            className="long-image-right-left-img pointer caption-spacing"
          />
        ) : null}
        {leftImage?.caption && (
          <div className="long-image-right-caption">
            <p className="type-detail-regular">{leftImage.caption}</p>
          </div>
        )}
      </div>
      <div className="long-image-right-right-image">
        {rightImage ? (
          <Image
            onClick={() =>
              setActiveImage({
            ...
            height={1920}
            alt={rightImage.alt || ""}
            className="long-image-right-right-img pointer caption-spacing"
          />
        ) : null}
        {rightImage?.caption && (
          <div className="long-image-right-caption">
